...
const requestHeaders = { Referer: 'https://bulls192.github.io/HotDogShopV29/' };
...
async function getDocument(path) {
  ...
  const json = await requestJson(url, path);
...
async function listCollection(collectionName) {
  ...
  let pageToken = '';
  do {
    const params = new URLSearchParams({ key: config.apiKey, pageSize: '300' });
    if (pageToken) params.set('pageToken', pageToken);
    const json = await requestJson(`${base}/${collectionName}?${params}`, collectionName);
    for (const doc of json.documents || []) {
      rows.push({
        id: String(doc.name || '').split('/').pop(),
        data: decodeCustom(decodeFields(doc.fields || {}))
      });
    }
...
const sourceEntities = [...sourceGroups.entries()].map(([id, records]) => ({
  ...
  records,
  representative: records.slice().sort((a, b) => nameOf(b.player).length - nameOf(a.player).length)[0]?.player || {}
}));
...
let canonCounter = 0;
for (const entities of exactGroups.values()) {
  canonCounter += 1;
  ...
  if (sourceIds.length > 1 && exact) exactDuplicates.push({ name: nameOf(representative), sourceIds, eventCount: eventIds.length });
}
...
for (const players of weakGroups.values()) {
  if (players.length > 1) possibleDuplicates.push({ names: players.map(p => p.name), sourceIds: [...new Set(players.flatMap(p => p.sourceIds))] });
}
...
function canonicalId(player, eventId) {
  ...
  return exactToCanonical.get(exactKey(player)) || null;
}
...
for (const result of resultRows) {
  if (!standings.has(result.canonicalId)) standings.set(result.canonicalId, { player: canonical.get(result.canonicalId)?.name || result.canonicalId, points: 0, played: 0, wins: 0, podiums: 0, lastPlayed: '' });
...
const eventTimeline = events.slice().sort((a, b) => b.date.localeCompare(a.date)).map(e => ({
  date: e.date,
  location: e.location,
  players: e.players.length,
  teams: e.teams.length,
  champion: teamName(e.champion),
  runnerUp: teamName(e.runnerUp),
  source: e.id
}));
...
const output = {
  ...
  collectionCounts: {
    ...
    seriesSeasonAvailable: Boolean(seriesDoc)
  },
